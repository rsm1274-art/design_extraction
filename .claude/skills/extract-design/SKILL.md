...
0. **Name the output folder after the site.** Derive a slug from the URL host:
   lowercase, drop `www.`, replace `.` with `-`, ignore the path.
   `https://stripe.com` -> `stripe-com`; `https://www.apple.com/mac` -> `apple-com`.
   Always pass it with `--out` so output never lands in `design-extract-output`.

1. **Run the extraction** on the provided URL:

```bash
npx designlang <url> --screenshots --out ./<site-slug>
```

For multi-page crawling: `npx designlang <url> --depth 3 --screenshots --out ./<site-slug>`
For dark mode: `npx designlang <url> --dark --screenshots --out ./<site-slug>`

2. **Read the generated markdown file** to understand the design:

```bash
cat <site-slug>/*-design-language.md
```
...
| Flag | Description |
|------|-------------|
| `--out <dir>` | Output directory. ALWAYS pass the site slug, e.g. `--out ./stripe-com` |
